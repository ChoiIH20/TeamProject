...
<div class="modal-dialog" role="document">
	<div class="modal-content rounded-5 shadow">
		<div class="modal-header p-5 pb-4 border-bottom-0">
			<h2 class="fw-bold mb-0">회원가입 페이지입니다.</h2>
			<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
				onclick="location.href='${pageContext.request.contextPath}/home.do'"></button>
		</div>
		<div class="modal-body p-5 pt-0">
			<form action="${pageContext.request.contextPath}/users/sign.do"	method="post" id="myForm">
				...
				<div class="form-floating mb-3">
					...
					<small class="form-text text-muted">영어 소문자와 숫자를 조합하여 5~10글자 이내로 입력하세요.</small>
...
<script>
	...
	document.querySelector("#id").addEventListener("input",	function(){
		...
		const self = this; //id의 참조값인 document.querySelector("#id")를 self로 담아서 사용 
		...
		//console.log(inputId);
		let reg_id = /^(?=.*[a-z])(?=.*[0-9])[A-Za-z0-9]{5,10}$/; //영문자 소문자로 시작하고 5글자 이상 10글자 이하로 된 아이디
		if (!reg_id.test(inputId)) { //inputId에 입력된 값과 정규 표현식이 같은지 비교한다
			isIdValid = false;
	...
	//input으로 입력되는 값이 제대로 입력되었는지 console에 값을 출력시킴
